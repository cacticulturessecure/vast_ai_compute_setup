...
class WhisperXProcessor:
    ...
    def load_metadata(self, wav_file: Path) -> Optional[Dict]:
        ...
        try:
            # Look for metadata JSON in the same directory as the audio file
            ...
            if metadata_file.exists():
                with open(metadata_file, 'r') as f:
                    ...
                    if 'attendees' in metadata:
                        speaker_count = len(metadata['attendees'])
                        logging.info(f"Found metadata with {speaker_count} speakers")
                        return {
            ...
            # If not found, try looking in a metadata subdirectory
            metadata_dir = wav_file.parent / "metadata"
            if metadata_dir.exists():
                metadata_file = metadata_dir / f"{wav_file.stem}_metadata.json"
                logging.info(f"Looking in metadata directory: {metadata_file}")
                if metadata_file.exists():
                    with open(metadata_file, 'r') as f:
                        metadata = json.load(f)
                        if 'attendees' in metadata:
            ...
            logging.warning(f"No metadata file found for {wav_file.name}")
            return None
